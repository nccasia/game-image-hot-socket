...
import { PVPRoomState, KeywordItem as ImportedKeywordItem, Player, TopicItem } from "./schema/PVPRoomState";
...
export class PVPRoomRankPrize extends Room<PVPRoomState> {
  ...
  private selectionSet: TopicItem[] = [];
  ...
  private broadcastQuestion(): void {
    if (this.currentQuestionIndex < this.maxQuestions) {
      ...
      this.currentChoiceList = currentTopic.keywords.map(k => k.Key);
  ...
  private loadAndPrepareSelectionSet(): void {
    ...
    for (let i = 0; i < this.maxQuestions; i++) {
      ...
      try {
        ...
        topicItem.topic = topicKey;

        selectedKeywordsData.forEach(kData => {
          const keyword = new ImportedKeywordItem();
          keyword.Key = kData.Key;
          topicItem.keywords.push(keyword);
        });
